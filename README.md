...
- PyTorch 기반 딥러닝 회귀 모델 개발
  - 체중 변화 예측
  - 영양 공식 기반 합성 데이터로 학습한 ML 예측 적용
  - ML 추론 적용 후 동시 요청 처리량이 낮아지는 한계 확인
...
- 목표 체중 대비 진척도 시각화
- 30일 후 체중 변화 예측
- 영양 공식으로 생성한 합성 데이터를 이용해 초기 학습 데이터 부족 보완
...
- PyTorch 기반 회귀 모델
- 영양 공식 기반 합성 데이터
- StandardScaler 전처리와 9개 사용자·식단 특성을 이용한 체중 변화 예측
...
### 9️⃣ ML 적용 후 동시 요청 처리량 저하

**문제**

- 체중 변화 예측 요청에서 Django 프로세스가 PyTorch 추론을 동기적으로 실행
- 규칙 계산만 수행하던 방식보다 요청 한 건의 연산량이 늘어 동시 요청 처리량이 낮아짐

**현재 구현**

- 모델과 StandardScaler를 최초 요청에 한 번만 로드한 뒤 프로세스 전역에서 재사용
- 최근 30일 평균 섭취량과 사용자 신체 정보를 9개 입력 특성으로 변환
- `torch.no_grad()`를 적용해 추론 과정의 불필요한 그래디언트 계산 제거

**결과**

- PyTorch 회귀 모델을 이용한 30일 후 체중 변화 예측 기능 구현
- 다만 동시 요청 처리 성능은 규칙 기반 방식보다 낮았으며 별도 추론 서버나 작업 큐 분리는 적용하지 못함
...
- 서버 중심 SPA 설계
- AI 연동 및 에러 디버깅 경험
- ML 기능을 웹 요청 흐름에 연결하고 동시성 한계를 확인한 경험
